...
void read_file(double x[POINTS], double y[POINTS]) {
    FILE *file = fopen("C:/labs/Control_work_5/MNK_input_data.txt", "r");
    if (file == NULL) {
        printf("Error");
        exit(1);
    }
...
void print_file(double x[POINTS], double y[POINTS]) {
    FILE *file = fopen("C:/labs/Control_work_5/MNK_answer_data.txt", "a");
...
void clear_file() {fclose(fopen("C:/labs/Control_work_5/MNK_answer_data.txt", "w"));}
...
void method_mnk() {
    ...
    file = fopen("C:/labs/Control_work_5/MNK_answer_data.txt", "a");
    fprintf(file, "\nFinish_system:\n");
    for(int i = 0; i < DEGREE + 1; i++) {
        if (array[i][i] == 0) {

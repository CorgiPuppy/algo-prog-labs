...
void print_array(double array[N][M]) {
    FILE *file = fopen("C:/labs/Control_work_5/SLAE_answer_data.txt", "a");
    char name = 'a';
    for(int i = 0; i < N; i++) {
        name = 'a';
...
void method_Gauss() {
    ...
    fclose(file);   
    print_array(array);
    file = fopen("C:/labs/Control_work_5/SLAE_answer_data.txt", "a");
...
void read_data(double array[N][M]) {
    FILE *file = fopen("C:/labs/Control_work_5/SLAE_input_data.txt", "r");
    if(file == NULL) {
        printf("Error");
        exit(1);
    }
...
void clear_file() {fclose(fopen("C:/labs/Control_work_5/SLAE_answer_data.txt", "w"));}